...
import { get_banner, get_menu, get_tc_menu, get_corporate_menu } from './component/select.js';
import { save_banner, save_menu, save_tc_menu, save_corporate_menu } from './component/insert.js';
import { update_default_banner, update_default_menu, update_edit_menu, update_default_tc_menu, update_edit_tc_menu, update_default_corporate_menu, update_edit_corporate_menu } from './component/update.js';
import { delete_banner, delete_menu, delete_tc_menu, delete_corporate_menu } from './component/delete.js';
...
app.post(process.env.SAVE_BANNER, upload.single('image_banner'), async (req, res) => {
    try {
        ...
        const filename = 'banner_'  + FileTime + ext;
...
app.post(process.env.SAVE_MENU, upload.single('image_menu'), async (req, res) => {
    try {
        const file = req.file;
        const ext = path.extname(file.originalname);
        const filename = 'menu_'  + FileTime + ext;
        const result = await save_menu(filename, file.size, req.body.menu_button);
        if (result === 'success') {
...
app.post(process.env.SAVE_TC_MENU, upload.single('image_tc_menu'), async (req, res) => {
    try {
        const file = req.file;
        const ext = path.extname(file.originalname);
        const filename = 'tc_menu_'  + FileTime + ext;
...
app.put(process.env.UPDATE_TC_CORPORATE_MENU, async (req, res) => {
    try {
...
app.post(process.env.SAVE_CORPORATE_MENU, upload.single('image_corporate_menu'), async (req, res) => {
    try {
        const file = req.file;
        const ext = path.extname(file.originalname);
        const filename = 'corporate_menu_'  + FileTime + ext;
...
app.get(process.env.GET_HOME, async (req, res) => {
    try {
        ...
        res.send({ result_banner, result_menu });
    } catch (error) {
        console.error(error);
    }
});
